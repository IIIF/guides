...
That's it!  Now when this HTML page loads, it will initialize the Leaflet map and include the FeatureCollection found in the `navPlace` property of this Manifest.  You can [go to the example page](example.html) to see the implementation on its own.  You should see a Leaflet load up, zoomed out pretty far, with a single greenish Point drawn in Germany. Click on the point to see your metadata!
